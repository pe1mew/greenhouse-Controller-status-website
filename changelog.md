...
- Dedup against future firmware: if the firmware ever starts emitting `sd_not_mounted` directly in `mode.flags`, the `includes()` guard in `renderMode()` keeps it from rendering twice.
- `mock/state.py` system defaults gain `sd_mounted: True`, `sd_free_mb: 1875`, `sd_size_mb: 1880` (matching the live values from pe1mew.nl); `mock/templates/control.html` adds a one-click `sd_mounted` ON/OFF toggle under the "System / uptime override" section so the badge can be exercised end-to-end without waiting for the controller's SD to actually unmount.
- `design/functional-design.md` and `design/apiSpecification.md` field tables updated with the three new fields and the synthetic-flag rule.
...
### Changed — `manual/md2pdf.py`: table-format metadata, intrinsic image sizing, header text (2026-05-24)
...
- **Header text updated** to `Kas controller - status page` (from `Kas Controller - Herenboeren Wenumseveld`) — the manual covers the status page generically, not one specific installation.
- Verified end-to-end with `python manual/md2pdf.py manual/userManual.md`. Latest output: `manual/userManual.pdf` (16 pages, ~585 KiB), rendered via Edge headless (Skia/PDF m148). Header now reads `Kas controller - status page … v1.0` on every page after the title page.
